...
- (void)viewDidLayoutSubviews
{
    ...
    if (@available(iOS 8.0, *)) {
        backButton.layoutMargins = UIEdgeInsetsMake(8, 10, 8, 10);
    ...
    if (self.searchViewControllerShowMode == PYSearchViewControllerShowModePush) {
        UIButton *backButton = self.navigationItem.leftBarButtonItem.customView;
        [backButton setTintColor:[UIColor colorWithHexString:@"#C1C1C1"]];
        UIImageView *imageView = backButton.imageView;

        [backButton sizeToFit];
        [imageView sizeToFit];

        backButton.py_height = 25;//navigationBar.py_height;
        backButton.py_width = 25;//titleLabel.py_width + imageView.py_width / 2.0 + backButtonLayoutMargins.left + backButtonLayoutMargins.right;
...
- (UIButton *)refeshButton
{
    if (!_refeshButton) {
        ...
        refeshButton.titleLabel.font = self.hotSearchHeader.font;
        [refeshButton setTitleColor:PYTextColor forState:UIControlStateNormal];
        [refeshButton setTitle:@"换一批" forState:UIControlStateNormal];
        [refeshButton setImage:[NSBundle py_imageNamed:@"refesh"] forState:UIControlStateNormal];
        [refeshButton addTarget:self action:@selector(refeshSearchDidClick) forControlEvents:UIControlEventTouchUpInside];
        [refeshButton sizeToFit];
        refeshButton.py_width += PYSEARCH_MARGIN;
        refeshButton.py_height += PYSEARCH_MARGIN;
        refeshButton.py_centerY = self.hotSearchHeader.py_centerY;
        refeshButton.py_x = self.hotSearchView.py_width - refeshButton.py_width;
        refeshButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
        [self.hotSearchView addSubview:refeshButton];
        _refeshButton = refeshButton;
    }
...
- (UIBarButtonItem *)backBarButtonItem
{
    ...
    self.backButton = wBarBtn;
    self.backBarButtonItem = wBarItem;
    return wBarItem;
}
...
//    backButton.frame = MQ_BARBUTTON_ITEM_FRAME;
//    [backButton setImage:MQ_IMAGE(@"back.png") forState:UIControlStateNormal];
    
    UIButton *backButton = [UIButton buttonWithType:UIButtonTypeSystem];
    UIImage *backImage = MQ_IMAGE(@"back.png");
...
//    [backButton sizeToFit];
    ...
    self.searchViewControllerShowMode = PYSearchViewControllerShowModePush;
...
//    self.refeshButton.py_y = self.hotSearchView.py_y - PYSEARCH_MARGIN * 0.5;
//    self.hotSearchTagsContentView.py_y = CGRectGetMaxY(self.refeshButton.frame) + PYSEARCH_MARGIN;
